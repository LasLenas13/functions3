...
// CODE HERE
const postTaxPrices = prices.map((afterTax) => {
  return afterTax * 1.07
}) 
...
const totals = (orders) => {
  let {price, tax} = orders;
  return price + (price * tax);
}

const orderTotals = orders.map(totals);
...
// CODE HERE
// first we want to select all of Bobs purchases
// next total up purchases
